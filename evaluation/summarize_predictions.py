...
METRIC_NAMES = ("accuracy", "balanced_accuracy", "precision", "recall", "f1", "auroc")


def metric_row(frame, experiment_name, scope):
...
def generator_rows(frame, experiment_name, scope):
    ...
    for label_b in range(1, 6):
        ...
        row, _ = metric_row(subset, experiment_name, scope)
        row.update({
            "label_b": label_b,
            "generator": str(fake["generator"].iloc[0]),
            "mean_fake_probability": float(fake["fake_prob"].mean()),
            "false_negatives": int(((fake["label"] == 1) & (fake["pred_label"] == 0)).sum()),
            "false_negative_rate": float((fake["pred_label"] == 0).mean()),
        })
        rows.append(row)
...
def summarize(predictions_path, original_metrics_path, balanced_manifest, experiment_name, out_dir, figure_dir):
    ...
    full_row, full_cm = metric_row(predictions, experiment_name, "official_full")
    assert_matches(full_row, original_metrics_path)
    balanced = select_manifest(predictions, balanced_manifest)
    balanced_row, balanced_cm = metric_row(balanced, experiment_name, "balanced_from_full_predictions")
    ...
    generator_rows(predictions, experiment_name, "official_full").to_csv(
        out_dir / f"{experiment_name}_full_by_generator.csv", index=False
    )
    generator_rows(balanced, experiment_name, "balanced_from_full_predictions").to_csv(
...
def main():
    ...
    full, balanced = summarize(
        ...
        args.experiment_name, args.out_dir, args.figure_dir,
    )
